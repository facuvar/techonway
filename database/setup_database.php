...
try {
    ...
    // Create database
    $pdo->exec("CREATE DATABASE IF NOT EXISTS techonway CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
    echo "Database 'techonway' created or already exists.<br>";
    
    // Select the database
    $pdo->exec("USE techonway");
    ...
    foreach ($queries as $query) {
        ...
        if (!empty($query)) {
            ...
            if (strpos($query, 'CREATE DATABASE') === false && strpos($query, 'USE techonway') === false) {
                $pdo->exec($query);
