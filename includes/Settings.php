...
class Settings {
    ...
    public function get($key, $default = null) {
        try {
            $row = $this->db->selectOne('SELECT `value_data` FROM settings WHERE `key_name` = ?', [$key]);
            if ($row && isset($row['value_data'])) {
    ...
    public function set($key, $value) {
        try {
            $exists = $this->db->selectOne('SELECT `key_name` FROM settings WHERE `key_name` = ?', [$key]);
            if ($exists) {
                $this->db->update('settings', ['value_data' => $value], '`key_name` = ?', [$key]);
            } else {
                // Evitar conflicto con palabra reservada `key`
                $this->db->query('INSERT INTO settings (`key_name`, `value_data`) VALUES (?, ?)', [$key, $value]);
            }
        } catch (Throwable $e) {
